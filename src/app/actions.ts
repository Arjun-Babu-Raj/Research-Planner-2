...
import {
  generatePisAndConsent,
  type GeneratePisAndConsentInput,
} from '@/ai/flows/generate-pis-consent';
...
export async function generatePisAndConsentAction(input: GeneratePisAndConsentInput & ActionInputWithApiKey) {
  try {
    const result = await generatePisAndConsent(input);
    return { success: true, data: result };
  } catch (error) {
    console.error(error);
    const errorMessage = error instanceof Error ? error.message : 'An unknown error occurred.';
    return { success: false, error: `Failed to generate PIS/Consent: ${errorMessage}` };
  }
}
