...
export async function generateStudySection(input: GenerateStudySectionInput & { apiKey: string }): Promise<GenerateStudySectionOutput> {
  ...
  const prompt = `You are an AI research assistant. Your task is to generate an initial draft for a specific section of a research study plan.

The title of the study is: "${input.studyTitle}"
The section you need to generate is: "${input.sectionType}"
...
`;
  ...
  const { output } = await ai.generate({
    ...
    prompt: prompt,
    output: {
      schema: GenerateStudySectionOutputSchema,
    }
  });
