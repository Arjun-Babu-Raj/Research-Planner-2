...
export async function refineStudySection(input: RefineStudySectionInput & { apiKey: string }): Promise<RefineStudySectionOutput> {
  ...
  const prompt = `You are an AI research assistant helping to refine sections of a study plan based on user feedback.

  The user is working on the "${input.sectionTitle}" section of their study plan. The current content of the section is:
  ...
  const {output} = await ai.generate({
    ...
    prompt,
    output: {
      schema: RefineStudySectionOutputSchema,
    }
  });
  return output!;
}
