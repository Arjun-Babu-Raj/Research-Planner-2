...
export async function clarifyStudyTitle(input: ClarifyStudyTitleInput & { apiKey: string }): Promise<ClarifyStudyTitleOutput> {
  ...
  const {output} = await ai.generate({
    ...
    prompt: `You are an AI assistant helping researchers clarify their study titles. Ask a few clarifying questions to ensure the study title is well-defined. Return your response as a string. Study Title: ${input.studyTitle}`,
    output: {
      schema: ClarifyStudyTitleOutputSchema,
    }
  });
  return output!;
}
