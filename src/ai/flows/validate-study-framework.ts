...
export async function validateStudyFramework(input: ValidateStudyFrameworkInput & { apiKey: string }): Promise<ValidateStudyFrameworkOutput> {
  ...
  const prompt = `You are an expert research methodologist.
...
Framework Name: ${input.frameworkName}
Framework Principles:
${input.frameworkPrinciples}
...
`;
  ...
  const {output} = await ai.generate({
    ...
    prompt,
    output: {
      schema: ValidateStudyFrameworkOutputSchema,
    }
  });
  return output!;
}
